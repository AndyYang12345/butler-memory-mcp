...
| `vendored/memory_policy.py` | `src/ai_butler_runtime/memory_policy.py` | 提取：第 48–97 行（`_SYNONYMS`…`ExtractedMemory`）+ 第 367–423 行（`LocalSemanticEncoder`/`recent_decay`），imports 重写 |
...
表名、列名、语义与上游完全一致（连同一数据库），**schema 迁移仍由
ai-butler-framework 的部署负责**（`ai-butler-db upgrade`）。
